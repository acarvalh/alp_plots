...
#  python scripts/drawcomp_afterBDT.py  -w 2 -b BM3 --ra 1
import json
import os
...
else :  iDir       = '/afs/cern.ch/work/a/acarvalh/codeCMSHHH4b/CMSSW_8_0_25/src/Analysis/hh2bbbb_limit_scan/BM' #'/lustre/cmswork/hh/alp_afterMVA/'
...
if which == -1:
    ...
    fractions = ['test','test']
    regions = ['sr', 'sr']
    if ra ==-1 : legList = ["signal (HH4b SM) - SR", "bkg (mixed data) - SR"]
    else : legList = ["ggHH4b "+args.bdt+" - SR", "Background - SR"]
    colorList = [632, 430]
    dofill = [True,True]
    oname = 'comp_'+samples[0]+samples[1]+'_afterBDT'
    ...
    fractions = ['test','']
    regions = ['ms','ms']
    legList = ["bkg (mixed data) - MS", "data - MS"]
    colorList = [430, 1]
    dofill = [True,False]
    oname = 'comp_'+samples[0]+samples[1]+'_afterBDT'
